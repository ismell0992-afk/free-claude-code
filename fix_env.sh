...
cat > "${REPO_ROOT}/.env.fixed" << 'COMPLETE'
# NVIDIA NIM Config
NVIDIA_NIM_API_KEY=""
...
# Replace the original .env with the fixed version
mv "${REPO_ROOT}/.env.fixed" "${REPO_ROOT}/.env"

echo "=== .env Fixed ==="
cat "${REPO_ROOT}/.env" | grep -E "^(OLLAMA_BASE_URL|MODEL_)" | grep -E "^!" || echo "Config fixed"